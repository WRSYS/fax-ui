...
func main() {
	...
	mux.HandleFunc("/mem-uploads/", app.requireAuth(app.handleMemUploadServe))

	// Serve uploaded files from disk if not using in-memory storage
	if !app.UploadInMemory {
		mux.Handle("/uploads/", app.requireAuth(func(w http.ResponseWriter, r *http.Request) {
			http.StripPrefix("/uploads/", http.FileServer(http.Dir(app.UploadDir))).ServeHTTP(w, r)
		}))
	}
